...
CREATE TABLE usuarios (
  ...
  nombres VARCHAR(50) NOTOTOS,
  apellidos VARCHAR(50) NOT NULL,
  telefono VARCHAR(9),
  direccion TEXT,
  tipo_documento VARCHAR(10) DEFAULT 'DNI',
  numero_documento VARCHAR(9) UNIQUE,
...
CREATE TABLE contactos_gestion (
  ...
  estado VARCHAR(20) DEFAULT 'pendiente' CHECK (estado IN ('PEDIENTE', 'RESPONDIDO', 'COMPLETADO'))
);
...
CREATE TABLE contactos_general (
  ...
  fecha_contacto TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
  estado VARCHAR(20) DEFAULT 'NUEVO' CHECK (estado IN ('NUEVO', 'EN_PROCESO', 'RESPONDIDO', 'CERRADO')) -- Consistencia en mayúsculas
);
